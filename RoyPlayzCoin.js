...
contract("RoyPlayzCoin", function(accounts) {
	...
	it("handles delegated token transfers", function() {
		return RoyPlayzCoin.deployed().then(function(instance) {
			...
			assert.equal(balance.toNumber(), 70, "deducts the amount from the sending account");
			return tokenInstance.balanceOf(toAccount);
		}).then(function(balance) {
			assert.equal(balance.toNumber(), 30, "adds the amount from the receiving account");
